...
class Chosen extends AbstractChosen
  ...
  result_do_highlight: (el) ->
    Chosen.prototype.currentHover = el;
    if el.length
  ...
  search_results_mouseover: (evt) ->
    target = if $(evt.target).hasClass "active-result" then $(evt.target) else $(evt.target).parents(".active-result").first()
    if target
      Chosen.prototype.currentHover = target
  ...
  select_delete_option: () ->
    currentlyHighlighted = Chosen.prototype.currentHover;
    if $.isFunction(@delete_option)
      Chosen.prototype.currentHover.slideToggle();
      @delete_option.call this, currentlyHighlighted.text()
    else
      Chosen.prototype.currentHover.slideToggle();
