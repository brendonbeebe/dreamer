...
// set graph variables
    ...
    /*
     * This directive expects the follow structure and when the drop down object
     * is clicked the button will change color and the dropdown will appear
     * dropdown
     * --.button
     * --.dropdown
     */
    .directive('donttriggermenuclose', function () {
    ...
    .directive('dropdown', function ($rootScope) {
        return function(scope, elem, attrs) {
            ...
            var button = elem.children('.btn');
            var dropdown = elem.children('.dropdown');
            var isopen = false;

            //If the correct children are not found then the lengths will be 0
            if(button.length == 0 || dropdown.length == 0){
                console.log("The dropdown object does not have the required children elements.");
                return;
            }
            elem.on('click', function (event) {


                //stopPropgation should not fire if the element click has attribute "allowclick"
                //This was needed because otherwise buttons inside of a dropdown couldn't have their own click events
                if(event.target.getAttribute("allowclick") == undefined){
                    if(!isopen){
                ...
                //Without this line the box will close if you try and select the text box.
                if(event.target.type != "text" && event.target.type != "textarea" && event.target.class != "allowClick"){
            ...
            //This event is broadcasted from app/js: run()
            scope.$on('event:hideMenu', function(event,e) {
                isopen = false;
                if(button.hasClass("blue")) {
                    button.removeClass("blue");
                    dropdown.fadeOut("fast");
                }
                $('.tooltip').animate({opacity:0},400, function () {
        ...
        return {
            ...
            link: function(scope, element, attrs) {
                ...
                element.on('click', function () {
                    ...
                    var description = tooltip.data('description');

                    // Top Middle
                    if (tooltip.hasClass('top-middle')) {
                        tooltip
                            .addClass()
                            .html(description + '<div class="arrow"></div>')
                            .css({
                                display: 'block',
                                top: element_position.top - tooltip.height() - 26,
                                left: element_position.left - (tooltip.width()/2) + (element.width()/2)
                            })
                ...
                scope.$on('event:hideMenu', function(event,e) {
                    //TODO: I commented out the following lines because button and dropdwon are not defined, not sure what this is for
//                    if(button.hasClass("blue")) {
//                        button.removeClass("blue");
//                        dropdown.fadeOut("fast");
//                    }
    ...
    //TODO: 'onfocustoggle' will be replaced by chosen plugin - refactor
    .directive('onfocustoggle', function() {
        return function(scope, elem, attrs) {
            elem.focus(function(){
...
module.directive('chosen',function($document, $parse){
    var linker = function(scope,element,attrs,ngModel,Restangular) {

        var list = attrs['chosen'];
        var object = attrs.ngModel;
        ...
        element.chosen({
            create_option: function(term){
                var chosen = this;
                var listName = scope.$eval(list);


                //Takes lists like types, salaries, years and converts them to singlur version for databa se
                var dataName = "";
                if(list.charAt(list.length-1) == "s"){
                    if(list == "salaries"){
                        dataName = "salary";
                    } else
                        dataName = list.substring(0,list.length-1);
                }else{
                    dataName = list;
                }

                var newEntry={};
                newEntry[dataName] = term;

                listName.post(newEntry).then(function(addedEntry) {
                    listName.getList().then(function(response){
                        scope[list] = response;
                        //Create item to be pushed to server via reset api
                        var newItemList = {};
                        newItemList["id"]= addedEntry[0]["id"];
                        newItemList[dataName] = addedEntry[0][dataName];
                        ...
                        if(attrs.multiple)
                            getter(scope).push(newItemList["id"]);
                        else
                            setter(scope,newItemList["id"]);
                    });
            ...
            delete_option: function(term){
                var splitted = term.split("_");
                var index = splitted[splitted.length -1];
                var listName = scope.$eval(list);

                listName[index].remove().then(function(){
                    listName.getList().then(function(response){
                        ...
                        if(listName[index].id == getter(scope) ){
                            setter(scope,"");
                        }
                        scope[list] = response;
                    });
...
//Custom harvest directive
////Create option -> Launch Colorbox with two fields [Name, Email] -> Two buttons [Create, Cancel]
////[Create] -> Posts new user, refreshes user list, adds users to member table.
module.directive('chosenforusersonly',function($document, $parse){
    var linker = function(scope,element,attrs,ngModel,Restangular) {

        var object = attrs.ngModel;

        scope.$on('event:refreshChosen', function(event,e) {
            element.trigger('liszt:updated');
        });
        //Watch the
        scope.$watch(object, function(){
        ...
        element.chosen({
            create_option: function(term){
                ...
                //Set scope.member.name
                //Launch colorbox
                var getter = $parse(object);
                var setter = getter.assign;

                scope.addMemberDetail(term);


            },
            create_option_text: 'Create New User'
...
module.directive('requiressubscription',function(userFactory){
    var linker = function(scope,element,attrs) {
        var requiressubscription;
        attrs.$observe('requiressubscription', function(value) {
            if (value) {
        ...
        scope.$watch( function () { return userFactory.user; }, function (data) {
            if(data == null)
                return;
            var user = data;
            if(user.subscriptions!=undefined && user.subscriptions[requiressubscription] != true)
                element.hide();
        })
    };
...
module.directive('requirespermission',function(userFactory){
    var linker = function(scope,element,attrs) {
        var permissionlevel;
        attrs.$observe('exactrequirespermission', function(value) {
            if (value) {
        ...
        scope.$watch( function () { return userFactory.user; }, function (data) {
            var user = data;
            if(user.roleLevel != undefined && user.roleLevel < permissionlevel)
                element.hide();
        })
    };
...
module.directive('exactrequirespermission',function(userFactory){
    var linker = function(scope,element,attrs) {
        var exactrequirespermission;
        attrs.$observe('exactrequirespermission', function(value) {
            if (value) {
                exactrequirespermission = value;
            }
        });
        scope.$watch( function () { return userFactory.user; }, function (data) {
            var user = data;
            if(user.roleLevel == exactrequirespermission)
                element.show();
            else
                element.hide();
        })
