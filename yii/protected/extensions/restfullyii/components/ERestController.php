<?php
class ERestController extends AController
{
	...
	public function beforeAction($event)
	{
        ...
        $this->restFilter = $this->addSecurityFilters();
	...
	public function filterRestAccessRules( $c )
	{
        ...
        //TODO: Add this below !Yii::app()->user->isGuest &&
		if($this->validateAjaxUser($this->action->id))
			$c->run();
		else
		{
			Yii::app()->errorHandler->errorAction = '/' . $this->uniqueid . '/error';
	...
	*/
	...
	protected function saveModel($model, $data) {
        ...
        if($this->restIncludeNested){
            ...
            if($return_array) {
                return $this->getModel()->findAllByPk($ids);
            }
            else
                return $this->getModel()->findAllByPk($ids[0]);
        }
	...
	public function allToArray($models)
	{
        ...
        //Added an if statement to use the scenario set in the model
        if(!empty($models[0])){
            $options = array('scenario' => $models[0]->scenario);
        } else {
		...
		{
            //The default scenario is update, if it's been changed we should use that instead

            if($models->scenario != "update"){
                $options = array('scenario' => $models->scenario);
            }

			$this->_attachBehaviors($models);
            //var_dump($models->safeAttributeNames);die;
	...
	******************************************************************************************
    ...
    public function doRestUpdate($id, $data) {
			...
			if (is_null($model)) {
					...
					$this->outputHelper(
						'Record Updated', $this->loadOneModel($id,false), 1
					);
			}
    }

    protected function saveOtherRelation($models){
        return;
    }
	/**
